...
//		unit variance, using annRan0() as the source of uniform
//deviates.
//
//				prob(x) = b/2 * exp(-b * |x|).
//
//		b is chosen to be sqrt(2.0) so that the variance of the
//Laplacian 		distribution [2/(b^2)] becomes 1.
//------------------------------------------------------------------------
...
//		Generates a Laplacian distribution (zero mean and unit
//variance).
//----------------------------------------------------------------------
...
//		unless new_clust = true.  This is so that subsequent calls
//generate 		points from the same distribution.  It follows, of course, that any
//		attempt to change the dimension or number of clusters without
//		generating new clusters is asking for trouble.
//
//		Note: Cluster centers are not generated by a call to
//uniformPts(). 		Although this could be done, it has been omitted for
...
//		and max_dim.  The points are evenly distributed among the
//clusters. 		For each cluster, we generate points uniformly distributed along 		the
//flat within the hypercube.
//
//		This is done as follows.  Each cluster is defined by a d-element
//		control vector whose components are either:
//
//				CO_FLAG indicating that this component is to be
//generated 						uniformly in [-1,1], 				x		a value other than CO_FLAG in
//the range [-1,1], 						which indicates that this coordinate is to be 						generated as x
//plus a Gaussian random deviation 						with the given standard deviation.
//
//		The number of zero components is the dimension of the flat,
//which 		is a random integer in the range from 1 to max_dim.  The points 		are
//disributed between clusters in nearly equal sized groups.
//
//		Note: Once cluster centers have been set, they are not changed,
//		unless new_clust = true.  This is so that subsequent calls
//generate 		points from the same distribution.  It follows, of course, that any
...
//		generating new clusters is asking for trouble.
//
//		To make this a bad scenario at query time, query points should
//be 		selected from a different distribution, e.g. uniform or Gaussian.
//
