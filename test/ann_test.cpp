...
//								coordinates are input from
//file <file>. 		gen_data_pts			Create a set of data points
//whose 								coordinates are generated from the 								current point distribution.
//
//		Building the tree:
//		------------------
//		build_ann				Generate an approximate nearest
//neighbor 								structure for the current data set, using 								the selected splitting
//rules.  Any existing 								tree will be destroyed.
//
//		Query Generation/Searching:
//		---------------------------
//		read_query_pts <file>	Create a set of query points whose
//								coordinates are input from
//file <file>. 		gen_query_pts			Create a set of query points
//whose 								coordinates are generated from the 								current point distribution.
//		run_queries <string>	Apply nearest neighbor searching to the
//								query points using the
//approximate nearest 								neighbor structure and the given search 								strategy. Possible
//strategies are: 									standard = standard kd-tree search 									priority = priority search
//
//		Miscellaneous:
//		--------------
//		output_label			Output a label to the output
//file. 		dump <file>				Dump the current structure to
//given file. 								(The dump format is explained further in 								the source file
//kd_tree.cc.)
//	  	load <file>				Load a tree from a data file
//which was 								created by the dump operation.	Any 								existing tree will be
//destroyed.
//
// Options:
// --------
...
// ----------------------------------------
//		split_rule <type>		Type of splitting rule to use in
//building 								the search tree.  Choices are: 									kd			= optimized
//kd-tree 									midpt		= midpoint split 									fair		= fair split 									sl_midpt
//= sliding midpt split 									sl_fair		= sliding fair split
//									suggest		=
//authors' choice for best 								The default is "suggest".  See the file 								kd_split.cc
//for more detailed information.
//
//		shrink_rule <type>		Type of shrinking rule to use in
//building 								a bd-tree data structure.  If "none" is 								given, then no shrinking is
//performed and 								the result is a kd-tree.  Choices are:
//									none		=
//perform no shrinking 									simple		= simple shrinking 									centroid	=
//centroid shrinking 									suggest		= authors' choice for best 								The default
//is "none".  See the file 								bd_tree.cc for more information. 		bucket_size <int>
...
//generation.
//		data_size <int>			Number of data points.  When
//reading data 								points from a file, this indicates the 								maximum number of points
//for storage 								allocation. Default = 100. 		query_size <int>		Same as
//data_size for query points.
//		std_dev <float>			Standard deviation (used in
//gauss, 								planted, and clustered distributions). 								This is the "small" distribution
//for 								clus_ellipsoids.  Default = 1. 		std_dev_lo <float>		Low and high
//standard deviations (used in
//		std_dev_hi <float>		clus_ellipsoids).  Default = 1.
//		corr_coef <float>		Correlation coefficient (used in
//co-gauss 								and co_lapace distributions). Default = 0.05. 		colors <int>
//Number of color classes (clusters) (used 								in the clustered distributions).
//Default = 5.
//		new_clust				Once generated, cluster centers
//are not 								normally regenerated.  This is so that both 								query points and data
//points can be generated 								using the same set of clusters.  This option 								forces
//new cluster centers to be generated 								with the next generation of either data or
//								query points.
//		max_clus_dim <int>		Maximum dimension of clusters (used
//in 								clus_orth_flats and clus_ellipsoids). 								Default = 1. 		distribution <string>
//Type of input distribution 									uniform		= uniform over cube [-1,1]^d.
//									gauss		=
//Gaussian with mean 0 									laplace		= Laplacian, mean 0 and var 1 									co_gauss
//= correlated Gaussian 									co_laplace	= correlated Laplacian
//									clus_gauss	=
//clustered Gaussian 									clus_orth_flats = clusters of orth flats 									clus_ellipsoids =
//clusters of ellipsoids 									planted		= planted distribution 								See the
//file rand.cpp for further information.
//
// Options affecting nearest neighbor search:
// ------------------------------------------
//		epsilon <float>			Error bound for approx. near
//neigh. search. 		near_neigh <int>		Number of nearest neighbors to
//compute. 		max_pts_visit <int>		Maximum number of points to visit before
//								terminating.  (Used in
//applications where 								real-time performance is important.) 								(Default = 0, which
//means no limit.) 		radius_bound <float>	Sets an upper bound on the nearest
//								neighbor search radius.  If
//the bound is 								positive, then fixed-radius nearest 								neighbor searching is
//performed, and the 								count of the number of points in the 								range is returned.  If
//the bound is 								zero, then standard search is used. 								This can only be used with
//standard, not 								priority, search.  (Default = 0, which 								means standard search.)
//
// Options affection general program behavior:
// -------------------------------------------
//		stats <string>			Level of statistics output
//									silent		 = no
//output, 									exec_time	+= execution time only 									prep_stats	+= preprocessing
//statistics 									query_stats += query performance stats 									query_res	+= results of
//queries 									show_pts	+= show the data points 									show_struct += print search
//structure 		validate <string>		Validate experiment and compute average
//								error.  Since validation
//causes exact 								nearest neighbors to be computed by the 								brute force method, this
//can take a long 								time.  Valid arguments are:
//									on			=
//turn validation on 									off			= turn validation off
//		true_near_neigh <int>	Number of true nearest neighbors to
//compute. 								When validating, we compute the difference 								in rank between each
//reported nearest neighbor 								and the true nearest neighbor of the same 								rank. Thus
//it is necessary to compute a 								few more true nearest neighbors.  By default 								we
//compute 10 more than near_neigh.  With 								this option the exact number can be
//set. 								(Used only when validating.)
//
// Example:
// --------
...
//	  stats query_stats			# output performance statistics for
//queries 	  seed 121212				# random number seed 	  data_size
//1000 	  distribution uniform
//	gen_data_pts				# 1000 uniform data points in dim
//16 	  query_size 100 	  std_dev 0.05 	  distribution clus_gauss 	gen_query_pts
//# 100 points in 10 clusters with std_dev 0.05 	  bucket_size 2 	  split_rule kd
//	  shrink_rule none
//	build_ann					# kd-tree, bucket size 2
//	  epsilon 0.1
//	  near_neigh 5
//	  max_pts_visit 100			# stop search if more than 100
//points seen
//	run_queries standard		# run queries; 5 nearest neighbors, 10%
//error 	  data_size 500
//	read_data_pts data.in		# read up to 500 points from file
//data.in 	  split_rule sl_midpt 	  shrink_rule simple 	build_ann
//# bd-tree; simple shrink, sliding midpoint split 	  epsilon 0 	run_queries
//priority		# run same queries; 0 allowable error
//
//------------------------------------------------------------------------
...
//		is allocated.
//
//		data_pts, query_pts				The data and query
//points
//		the_tree						Points to the kd- or
//bd-tree for 										nearest neighbor searching. 		apx_nn_idx, apx_dists
//Record approximate near neighbor 										indices and distances 		apx_pts_in_range
//Counts of the number of points in 										the in approx range, for fixed- 										radius NN
//searching. 		true_nn_idx, true_dists			Record true near
//neighbor 										indices and distances 		min_pts_in_range, max_...		Min and
//max counts of the number 										of points in the in approximate 										range. 		valid_dirty
//To avoid repeated validation, 										we only validate query results 										once.  This
//validation becomes 										invalid, if a new tree, new data 										points or new query points
//have 										been generated.
//		tree_data_size					The number of points
//in the 										current tree.  (This will be the 										same a data_size unless points have
//										been added since
//the tree was 										built.)
//
    ...
    istream &in)     // input stream
{
  ...
  if (in.eof())
    return ANNfalse; // end of file
  in.putback(ch);    // put character back
  return ANNtrue;
}

ANNbool getDirective(istream &in,     // input stream
...
int main(int argc, char **argv) {
  ...
  while (getDirective(cin, directive)) {
    ...
    //----------------------------------------------------------------
      ...
      if (distr == PLANTED) { // planted distribution
        if (data_pts == NULL) {
          Error("Must generate data points before query points for planted "
                "distribution",
                ANNabort);
        }
    ...
    //----------------------------------------------------------------
      ...
      if (stats > SILENT) {
        ...
        if (stats >= EXEC_TIME) { // output processing time
          cout << "  process_time  = " << double(prep_time) / CLOCKS_PER_SEC
               << " sec\n";
        }
    ...
    //----------------------------------------------------------------
      ...
      if (!strcmp(arg, "standard")) {
        ...
        Error("Search type must be \"standard\" or \"priority\"", ANNabort);
      }
      ...
      if (apx_dists != NULL)
        delete[] apx_dists;
      if (apx_pts_in_range != NULL)
        delete[] apx_pts_in_range;
      ...
      for (int i = 0; i < query_size; i++) {
        ...
        if (radius_bound == 0) { // no radius bound
          ...
          if (method != STANDARD) {
            Error("A nonzero radius bound assumes standard search", ANNwarn);
          }
      ...
      if (validate) { // validation requested
        if (valid_dirty)
          getTrueNN();  // get true near neighbors
      ...
      if (stats > SILENT) {
        ...
        if (stats >= QUERY_RES) { // output results
          ...
          for (int i = 0; i < query_size; i++) {
            ...
            // output range count
            if (radius_bound != 0) {
              cout << "    pts_in_radius_bound = " << apx_pts_in_range[i]
                   << "\n";
            }
...
void generatePts(ANNpointArray &pa, // point array (returned)
                 ...
                 int n_src)         // source size (if distr=PLANTED)
{
  ...
  case CLUS_ELLIPSOIDS: // clustered ellipsoids
    annClusEllipsoids(pa, n, dim, n_color, new_clust, std_dev, std_dev_lo,
                      std_dev_hi, max_dim);
    break;
  ...
  if (stats > SILENT) {
    if (type == DATA)
      cout << "[Generating Data Points:\n";
    else
      cout << "[Generating Query Points:\n";
    cout << "  number        = " << n << "\n";
    cout << "  dim           = " << dim << "\n";
    cout << "  distribution  = " << distr_table[distr] << "\n";
    if (annIdum < 0)
      cout << "  seed          = " << annIdum << "\n";
    if (distr == GAUSS || distr == CLUS_GAUSS || distr == CLUS_ORTH_FLATS)
...
void readPts(ANNpointArray &pa, // point array (returned)
             int &n,            // number of points
             char *file_nm,     // file name
             PtType type)       // point type (DATA, QUERY)
{
...
//------------------------------------------------------------------------
//	getTrueNN
//		Computes the true nearest neighbors.  For purposes of
//validation, 		this intentionally done in a rather dumb (but safe way), by
...
//		new query or data points are generated, or if the requested
//number 		of true near neighbors or approximate near neighbors has changed.
//
...
{
  if (stats > SILENT) {
    cout << "(Computing true nearest neighbors for validation.  This may take "
            "time.)\n";
  }
  // deallocate existing storage
  if (true_nn_idx != NULL)
  ...
  if (true_dists != NULL)
    delete[] true_dists;
  if (min_pts_in_range != NULL)
    delete[] min_pts_in_range;
  if (max_pts_in_range != NULL)
    delete[] max_pts_in_range;
  ...
  for (int i = 0; i < query_size; i++) {
    if (radius_bound == 0) { // standard kNN search
      ...
      min_pts_in_range[i] =
          the_brute->annkFRSearch(query_pts[i], // query point
...
//		it assumed that the rank of the true nearest neighbor is
//true_nn+1.
//
//		Because of the possibility of duplicate distances, this is
//computed 		as follows.  For the j-th reported nearest neighbor, we count the
//		number of true nearest neighbors that are at least this close.
//Let 		this be rnk.  Then the rank error is max(0, j-rnk).  (In the code 		below, j
//is an array index and so the first item is 0, not 1.  Thus 		we take max(0,
//j+1-rnk) instead.)
//
...
void doValidation() // perform validation
{
  ...
  if (true_nn < near_neigh) {
    Error("Cannot validate with fewer true near neighbors than actual",
          ANNabort);
  }
  ...
  for (i = 0; i < query_size; i++) { // validate each query
    ...
    for (j = 0; j < near_neigh; j++) {
      ...
      if (resultErr > epsilon + RND_OFF && max_pts_visit == 0) {
        Error("INTERNAL ERROR: Actual error exceeds epsilon", ANNabort);
      }
#ifdef ANN_PERF
...
//--------------------------------------------------------------------
    ...
    if (radius_bound != 0) { // fixed-radius search
      if (apx_pts_in_range[i] < min_pts_in_range[i] ||
        ...
        Error("INTERNAL ERROR: Invalid fixed-radius range count", ANNabort);
    }
...
void treeStats(ostream &out,    // output stream
               ANNbool verbose) // print stats
{
  ...
  if (verbose) { // output statistics
    ...
    out << "    n_nodes          = " << n_nodes << " (opt = " << opt_n_nodes
        << ", best if < " << too_many_nodes << ")\n"
        << "        n_leaves     = " << st.n_lf << " (" << st.n_tl
        << " contain no points)\n"
        << "        n_splits     = " << st.n_spl << "\n"
        << "        n_shrinks    = " << st.n_shr << "\n";
    out << "    empty_leaves     = " << frac_tl * 100 << " percent (best if < "
        << MAX_FRAC_TL * 100 << " percent)\n";
    out << "    depth            = " << st.depth << " (opt = " << opt_levels
        << ", best if < " << too_many_levels << ")\n";
    out << "    avg_aspect_ratio = " << st.avg_ar << " (best if < "
        << MAX_AVG_AR << ")\n";
    out << "  )\n";
  }
}
